    ...
    for (finger_id, coords) in map {
        ...
        while i < potential_double_taps.len() {
            ...
            if timestamp - earlier_touch_timestamp > 0.45 {
                ...
                continue;
            } else if earlier_touch_location.distance_from_squared(location) < 1600.0 {
                tap_count += 1;
            }
            i += 1;
        }
...
fn handle_touches_up(env: &mut Environment, map: HashMap<FingerId, Coords>) {
    ...
    for (finger_id, coords) in map {
        ...
        if timestamp - initial_timestamp < 0.75
            && location.distance_from_squared(initial_location) < 625.0
        {
