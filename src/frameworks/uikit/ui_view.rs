...
use crate::frameworks::core_animation::{ca_layer, ca_transaction};
...
const _touchHLE_kCATransactionAnimationContext: &str = "_touchHLE_kCATransactionAnimationContext";
const _touchHLE_kCATransactionAnimationDelay: &str = "_touchHLE_kCATransactionAnimationDelay";
const _touchHLE_kCATransactionAnimationRepeatCount: &str =
    "_touchHLE_kCATransactionAnimationRepeatCount";
const _touchHLE_kCATransactionAnimationRepeatAutoreverses: &str =
    "_touchHLE_kCATransactionAnimationRepeatAutoreverses";
const _touchHLE_kCATransactionAnimationDelegate: &str = "_touchHLE_kCATransactionAnimationDelegate";
const _touchHLE_kCATransactionAnimationWillStartSelector: &str =
    "_touchHLE_kCATransactionAnimationWillStartSelector";
const _touchHLE_kCATransactionAnimationDidStopSelector: &str =
...
+ (())setAnimationDelay:(NSTimeInterval)delay {
    ...
    () = msg_class![env; CATransaction setValue:value forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDelay))];
}
...
+ (())setAnimationRepeatAutoreverses:(bool)repeat_autoreverses {
    ...
    () = msg_class![env; CATransaction setValue:value forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationRepeatAutoreverses))];
}
...
+ (())setAnimationRepeatCount:(f32)repeat_count {
    ...
    () = msg_class![env; CATransaction setValue:value forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationRepeatCount))];
}

+ (())setAnimationDelegate:(id)delegate {
    log_dbg!("[UIView setAnimationDelegate:{:?}]", delegate);
    retain(env, delegate);
    () = msg_class![env; CATransaction setValue:delegate forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDelegate))];
}
...
+ (())setAnimationWillStartSelector:(SEL)selector {
    ...
    () = msg_class![env; CATransaction setValue:selector_nsstring forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationWillStartSelector))];
}
...
+ (())setAnimationDidStopSelector:(SEL)selector {
    ...
    let selector_nsstring = from_rust_string(env, selector_str.to_string());
    () = msg_class![env; CATransaction setValue:selector_nsstring forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDidStopSelector))];
}
...
+ (())beginAnimations:(id)animation_id // NSString*
    ...
    () = msg_class![env; CATransaction setValue:animation_id forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationId))];
    if !context.is_null() {
        let context: id = msg_class![env; NSNumber numberWithUnsignedInt:(context.to_bits())];
        () = msg_class![env; CATransaction setValue:context forKey:(get_static_str(env, _touchHLE_kCATransactionAnimationContext))];
    }
    // Default values
    () = msg_class![env; UIView setAnimationDuration:0.2];
    () = msg_class![env; UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];

    // Enable implicit animations on all layers
    env.framework_state.uikit.ui_view.animation_block_count += 1;
    for view in env.framework_state.uikit.ui_view.views.clone() {
        let layer: id = msg![env; view layer];
        ca_layer::set_use_implicit_animations(env, layer, true);
    }
}
...
+ (())commitAnimations {
    ...
    let delegate: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDelegate))];
    let animation_delegate = if delegate == nil {
        ...
        let animation_id: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationId))];
        () = msg![env; animation_delegate setAnimationId:animation_id];
        let context: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationContext))];
        if context != nil {
        ...
        let will_start_selector: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationWillStartSelector))];
        ...
        let did_stop_selector: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDidStopSelector))];
    ...
    let delay: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationDelay))];
    let repeat_count: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationRepeatCount))];
    let repeat_autoreverses: id = msg_class![env; CATransaction valueForKey:(get_static_str(env, _touchHLE_kCATransactionAnimationRepeatAutoreverses))];
    ...
    // Disable implicit animations on all layers if there are no more active
    // UIView animation blocks.
    if env.framework_state.uikit.ui_view.animation_block_count == 0 {
        for view in env.framework_state.uikit.ui_view.views.clone() {
            let layer: id = msg![env; view layer];
            ca_layer::set_use_implicit_animations(env, layer, false);
        }
...
- (())dealloc {
    let UIViewAnimationDelegateHostObject {animation_id, delegate, ..} = *env.objc.borrow::<UIViewAnimationDelegateHostObject>(this);
...
- (())animationDidStart:(id)animation { // CAAnimation*
    let UIViewAnimationDelegateHostObject {started_animation_count, delegate, will_start_selector, context, animation_id, ..} = *env.objc.borrow::<UIViewAnimationDelegateHostObject>(this);
...
- (())animationDidStop:(id)animation // CAAnimation*
    ...
    let UIViewAnimationDelegateHostObject {total_animation_count, finished_animation_count, delegate, did_stop_selector, context, animation_id, ..} = *env.objc.borrow::<UIViewAnimationDelegateHostObject>(this);
